<x-layout>
    ...
    <div class="flex flex-wrap min-h-[70vh] mb-2.5">
        ...
        <div class="w-full md:w-1/3 p-4 h-[70vh]">
            <div class="border rounded-lg p-6 h-full flex flex-col dark:bg-white shadow-sm dark:border-gray-700">
                <h3 class="text-2xl font-bold text-center dark:text-gray-800">Your Assignments</h3>
                <hr class="my-4 h-px bg-gray-200 border-0 dark:bg-gray-700">

                <div class="overflow-y-auto flex-1 -mx-2 px-2 space-y-4">
                    @foreach($assignments as $assignment)
                        <a href="/assignment/{{ $assignment->assignment_id }}" class="block rounded-xl border border-gray-200 dark:border-gray-600 p-5 dark:hover:bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer shadow-sm">
                            <div class="flex justify-between items-start">
                                <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-900">
                                    {{ $assignment->title }}
                                </h4>
                                <span class="text-xs font-medium px-2 py-1 rounded-full
                                    {{ $assignment->isDeadlinePassed()
                                        ? 'dark:bg-red-100 dark:text-red-800 bg-red-900/50 text-red-200'
                                        : 'dark:bg-blue-100 dark:text-blue-800 bg-blue-900/50 text-blue-200' }}">
                                    {{ $assignment->isDeadlinePassed() ? 'Overdue' : 'Active' }}
                                </span>
                            </div>
                            ...
                            <p class="text-sm mt-2
                                ...
                                @if($assignment->deadline)
                                    <span>Due: {{ $assignment->deadline->format('M j, Y g:i A') }}</span>
                                    @unless($assignment->isDeadlinePassed())
                                        <span class="ml-1">({{ $assignment->deadline->diffForHumans() }})</span>
                                    @endunless
                                @else
                                    No deadline
                                @endif
                ...
                @if($assignments->isEmpty())
                    <div class="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400">
                        ...
                        <p class="text-sm mt-1">Your lecturer hasn't assigned any work</p>
                    </div>
                @endif
            </div>
        </div>
